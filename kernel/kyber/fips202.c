...
void shake128_inc_init(shake128incctx *state) {
    state->ctx = pi_l2_malloc(PQC_SHAKEINCCTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
void shake128_inc_ctx_clone(shake128incctx *dest, const shake128incctx *src) {
    dest->ctx = pi_l2_malloc(PQC_SHAKEINCCTX_BYTES);
    if (dest->ctx == NULL) {
        exit(111);
    }
...
void shake128_inc_ctx_release(shake128incctx *state) {
    pi_l2_free(state->ctx, PQC_SHAKECTX_BYTES);
}
...
void shake128_absorb(shake128ctx *state, const uint8_t *input, size_t inlen) {
    state->ctx = pi_l2_malloc(PQC_SHAKECTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
...
void shake128_ctx_clone(shake128ctx *dest, const shake128ctx *src) {
    dest->ctx = pi_l2_malloc(PQC_SHAKECTX_BYTES);
    if (dest->ctx == NULL) {
        exit(111);
    }
